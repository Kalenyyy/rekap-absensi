    ...
    <li>
        <div class="flex items-center">
            ...
            <a href="{{ route('admin.data-master.index-rombel') }}"
                class="ml-1 text-sm font-medium text-gray-700 hover:text-gray-900 md:ml-2 dark:text-gray-400 dark:hover:text-white">Rombel</a>
        </div>
...
@section('modal')
    <!-- Modal Rombel -->
    <div id="rombel-modal" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                    ...
                    <button type="button"
                        ...
                        data-modal-toggle="rombel-modal">
                ...
                <!-- Modal body -->
                <form class="p-4 md:p-5" action="{{ route('admin.data-master.store-rombel') }}" method="post">
                    @csrf
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2">
                            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                                Rombel</label>
                            <input type="text" name="name_rombel" id="name"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="Type Rombel name" required="">
                        </div>
                    </div>
                    <button type="submit"
    ...
    <!-- Modal edit rombel -->
    <div id="edit-rombel-modal" tabindex="-1" aria-hidden="true"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <!-- Modal body -->
                <form class="p-4 md:p-5" id="rombelForm" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2">
                            <label for="name_rombel"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
                            <input type="text" name="name_rombel" id="name_rombel"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
    ...
    {{-- Modal Delete Rombel --}}
    <div id="delete-rombel-modal" tabindex="-1"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <button type="button"
...
@section('content')
            ...
            <div
                ...
                <form id="search-form" action="{{ route('admin.data-master.index-rayon') }}" method="GET"
                    ...
                    <div class="relative flex mx-3">
                        ...
                        <input type="text" id="table-search-users" name="search_rombel"
                            class="block pt-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="Search for Rombels">
                    </div>
                </form>
                <button type="button" id="defaultModalButton" data-modal-target="rombel-modal"
                    data-modal-toggle="rombel-modal"
                        ...
                        <tr
                            ...
                            <td class="px-6 py-4">
                                ...
                                <a type="button" id="{{ $rombel->id }}" data-modal-target="edit-rombel-modal"
                                    data-modal-toggle="edit-rombel-modal"
                                    class="edit-rombel font-medium text-blue-600 dark:text-blue-500 hover:underline cursor-pointer">Edit
                                    | </a>
                                <a type="button" id="{{ $rombel->id }}" data-modal-target="delete-rombel-modal"
                                    data-modal-toggle="delete-rombel-modal"
                                    class="delete-rombel font-medium text-blue-600 dark:text-blue-500 hover:underline cursor-pointer">Delete</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const buttons = document.querySelectorAll('.edit-rombel');
            const deleteButtons = document.querySelectorAll('.delete-rombel');
            const modal = document.getElementById('edit-rombel-modal');
            const rombel = document.getElementById('name_rombel');
            const rombelForm = document.getElementById('rombelForm');

            buttons.forEach(button => {
                button.addEventListener('click', function(event) {
                    event.preventDefault(); // Prevent default action
                    const rombelId = this.getAttribute('id');
                    ...
                    $.ajax({
                        url: `http://127.0.0.1:8000/data-master/edit-rombel/${rombelId}`,
                        method: 'GET',
                        dataType: 'json',
                        success: function(response) {
                            console.log(response);
                            rombel.value = response.rombel.name_rombel;
                        },
